function Table_4(varargin)
    ...
    %
    % In matlab command window, run: "Table_4(number of cores)"
    % Example, put "Table_4(12)" in command window and press enter
    %
    ...
    %
    % Last Change Date: 03/June/2025
%%
    rngSeed = 420;  % Master seed
    ...
    %%
    logname = 'emp_20240909.txt';
    if exist(logname)
        diary off
        delete(logname);
    end
    diary('emp_20240909.txt');
    ...
    while (Loop_control == 1)
        ...
        %stream = parallel.pool.Constant(RandStream('Threefry', 'Seed', 41));
     
        parfor k = 1 : Length_L1
            stream = RandStream('Threefry', 'Seed', rngSeed + b + 10000000); % Unique seed per iteration
            RandStream.setGlobalStream(stream);  

            % localStream = stream.Value;
            % localStream.Substream = k;  % Different substream for each iteration
            %rng(k); 
...
%%%%% may 29 change %%%%%
    ...
    while Loop_control == 2
        ...
        parfor k = 1 : Length_L2
            stream = RandStream('Threefry', 'Seed', rngSeed + b + 2000000); % Unique seed per iteration
            RandStream.setGlobalStream(stream);  
    ...
    while Loop_control == 3
        ...
        %     Q_gr(k) = Qfunc(dX, dE, b_gr(k,:)'); % use Qfunc in this same folder!!
        % end
        % stream = parallel.pool.Constant(RandStream('Threefry', 'Seed', 43));

        parfor k = 1 : Length_L3
            stream = RandStream('Threefry', 'Seed', rngSeed + b + 3000000); % Unique seed per iteration
            RandStream.setGlobalStream(stream);  
            % localStream = stream.Value;
            % localStream.Substream = k;  % Different substream for each iteration 
            %rng(k); 
    ...
    beta_B
    % Save beta_B to a .mat file
    save('beta_B.mat', 'beta_B');
    diary off
