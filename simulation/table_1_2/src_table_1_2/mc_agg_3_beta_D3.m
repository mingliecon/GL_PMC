...
function[beta_B, beta0]=mc_agg_3_beta_D3(N,D,J,T,B,B_first,B_last,core,beta0,theta0,dX_B,dEy_B)
    ...
    parfor b = 1:B
        stream = RandStream('Threefry', 'Seed', rngSeed + b + 71800000); % Unique seed per iteration
        ...
        %
        %         bdry = boundary(theta_notmin(:,1),theta_notmin(:,2),0);
        %         plot(theta_notmin(bdry,1),theta_notmin(bdry,2),'k','LineWidth',2)
        %
        ...
        %check_b = min(min(theta0 <= theta_notmin_u), min(theta0 >= theta_notmin_l));
...
%clearvars -except N D J T B beta0 theta0 theta_B theta_out_B beta_B beta_out_B check_B B_first B_last
%eval(sprintf('save Sim_%dD_results_%d_%d_N=%d',D,B_first,B_last,N))
