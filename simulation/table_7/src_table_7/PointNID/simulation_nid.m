function MC_AGG_20230909_nlogn(varargin)
    ...
    parfor b = 1:B
        stream = RandStream('Threefry', 'Seed', rngSeed + b + 150000); % Unique seed per iteration
        ...
        %X(:, 1, :, :) =  randi([-10,10],[N, J, T]) / 10;
        X(:, 2, :, :) = 2*sqrt(6) * rand(N,J,T) - sqrt(6) + repmat(Z_i, 1, J, T);
        %X(:, 3:D, :, :) = 2 * rand(N, D-2, J, T) - 1;
        % For X1 (first dimension)
        X(:, 1, :, :) = (rand([N, J, T]) > 0.5) * 1.0 - 0.5;  % 50% chance of 0.5 or -0.5
        
        % For X3:D (remaining dimensions)
        X(:, 3:D, :, :) = (rand([N, D-2, J, T]) > 0.5) * 1.0 - 0.5;
    ...
    parfor b = 1: B
        stream = RandStream('Threefry', 'Seed', rngSeed + b + 160000); % Unique seed per iteration
        RandStream.setGlobalStream(stream); 
    ...
    parfor b = 1:B
        ...
        stream = RandStream('Threefry', 'Seed', rngSeed + b + 170000); % Unique seed per iteration
        RandStream.setGlobalStream(stream); 
    ...
    filename = sprintf('nlogn_%s_result.mat', nlogn_str);
